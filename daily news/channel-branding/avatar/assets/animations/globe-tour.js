...
function createRouteLayer(points, radius = EARTH_RADIUS + 0.045) {
...
export function createGlobeTour(options) {
  ...
  const legs = stops.map((stop, index) => {
    ...
    const primary = createCountryFillLayer(
      feature,
      {
    ...
    const relatedLayers = relatedCodes.map((code, relatedIndex) => {
  ...
  const routeLayers = legs.map((leg) => createRouteLayer(leg.routePoints));
  ...
  function renderAt(time) {
    ...
    legs.forEach((leg, index) => {
      ...
      if (route) {
        ...
        route.markerMaterial.opacity = routeOpacity * 0.94;
