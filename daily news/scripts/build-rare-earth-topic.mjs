...
/* Build a researched current-affairs episode into the reusable 16:9 rare-earth
 * landscape composition. Mirrors build-trade-topic.mjs but targets the
 * rare-earth-explainer project. */
...
const die = (message) => {
  console.error(`✗ rare-earth topic: ${message}`);
  process.exit(1);
};
...
const storyPath = resolveProjectPath(option("story", "stories/landscape-2026-08-05-rare-earth-leverage.json"));
const projectPath = resolve(projectRoot, "rare-earth-explainer");
const compositionPath = resolve(projectPath, "index.html");
const tempCompositionPath = resolve(projectPath, "index.__render.html");
const storyVariablesPath = resolveProjectPath(option("variables-output", "assets/narration/rare-earth-explainer-variables.json"));
const outputPath = resolveProjectPath(option("output", "renders/landscape-2026-08-05-rare-earth-leverage.mp4"));
