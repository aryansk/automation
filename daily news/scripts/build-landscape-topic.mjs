...
/* Build a researched current-affairs episode into the reusable 16:9 landscape composition. */
...
const die = (message) => {
  console.error(`✗ landscape topic: ${message}`);
  process.exit(1);
};
...
const storyPath = resolveProjectPath(option("story", "stories/landscape-2026-08-03-open-models-markets.json"));
const projectPath = resolve(projectRoot, "landscape-trend-explainer");
const compositionPath = resolve(projectPath, "index.html");
const tempCompositionPath = resolve(projectPath, "index.__render.html");
const storyVariablesPath = resolveProjectPath(option("variables-output", "assets/narration/landscape-trend-explainer-variables.json"));
const outputPath = resolveProjectPath(option("output", "renders/landscape-2026-08-03-open-models-markets.mp4"));
...
writeFileSync(
  ...
  `${JSON.stringify({
    ...
    ctaLine: story.ctaLine || "The next bottleneck is the story.",
