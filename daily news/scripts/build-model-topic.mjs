 ...
 * targets the model-explainer project. */
...
const die = (message) => {
  console.error(`✗ model topic: ${message}`);
  process.exit(1);
};
...
const storyPath = resolveProjectPath(option("story", "stories/landscape-2026-08-07-model-not-product.json"));
const projectPath = resolve(projectRoot, "model-explainer");
const compositionPath = resolve(projectPath, "index.html");
const tempCompositionPath = resolve(projectPath, "index.__render.html");
const storyVariablesPath = resolveProjectPath(option("variables-output", "assets/narration/model-explainer-variables.json"));
const outputPath = resolveProjectPath(option("output", "renders/landscape-2026-08-07-model-not-product.mp4"));
...
writeFileSync(
  ...
  `${JSON.stringify({
    ...
    ctaLine: "The moat is the system, not the model.",
