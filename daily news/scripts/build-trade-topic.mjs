...
/* Build a researched current-affairs episode into the reusable 16:9 trade
 * landscape composition. Mirrors build-landscape-topic.mjs but targets the
 * trade-trend-explainer project. */
...
const die = (message) => {
  console.error(`✗ trade topic: ${message}`);
  process.exit(1);
};
...
const storyPath = resolveProjectPath(option("story", "stories/landscape-2026-08-04-trade-trust.json"));
const projectPath = resolve(projectRoot, "trade-trend-explainer");
const compositionPath = resolve(projectPath, "index.html");
const tempCompositionPath = resolve(projectPath, "index.__render.html");
const storyVariablesPath = resolveProjectPath(option("variables-output", "assets/narration/trade-trend-explainer-variables.json"));
const outputPath = resolveProjectPath(option("output", "renders/landscape-2026-08-04-trade-trust.mp4"));
...
writeFileSync(
  ...
  `${JSON.stringify({
    ...
    ctaLine: "The route is the story.",
