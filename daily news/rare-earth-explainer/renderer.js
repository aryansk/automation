...
const compositionId = root?.dataset.compositionId || "landscape-trend-explainer";
...
document.getElementById("opener-title-top").textContent = variables.openerTop || story.openerTitle?.split("/")[0]?.trim() || "OPEN MODELS";
document.getElementById("opener-title-bottom").textContent = variables.openerBottom || story.openerTitle?.split("/").slice(1).join("/").trim() || "CLOSED PROFITS?";
const globeCaptionCopy = document.querySelector("#globe-caption .globe-caption__copy");
if (globeCaptionCopy) globeCaptionCopy.textContent = story.globeCaption || "A software decision becomes a global market question";
